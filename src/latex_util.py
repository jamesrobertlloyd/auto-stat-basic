...
def get_latex_template(templatefl):
    ...
    templateEnv.filters['escape_tex'] = escape_tex # remove latex from strings passed to template

    template = templateEnv.get_template(templatefl)
...
def error_pdf(latex_dir, templatefl = "./pdf_template.jinja", 
    ...
    update_tex(temp_dir, template.render({
                'title' : title,
                'body' : body}))
